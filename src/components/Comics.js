...
function urlGeneratorForSearch(offset, titleStartsWith) {
    ...
    const stringToHash = ts + privatekey + publickey;
    const hash = md5(stringToHash);
    const baseUrl = 'https://gateway.marvel.com:443/v1/public/comics?titleStartsWith=' + titleStartsWith + '&limit=' + limit+ '&offset=' + offset;
    const url = baseUrl + '?ts=' + ts + '&apikey=' + publickey + '&hash=' + hash;
    //console.log(url);
    return url;
    //console.log(url);
        ...
        <Grid item xs={12} sm={6} md={4} lg={3} xl={2} key={comic.id}>
            <Card className={classes.card} variant="outlined">
                ...
                <CardMedia
                    ...
                    component = "img"
                    image = {
                     comic.thumbnail.path && comic.thumbnail.extension ? comic.thumbnail.path + "." + comic.thumbnail.extension : noImage
                    }
                    title="show image"
                />
            ...
            return (
                <div>
                    <br />
                        {
                            (currentPage === 0) ? "": 
                                <Link  className = "paginationLink" to={`/comics/page/${currentPage - 1}`} onClick={onClickPrev}>
                                Previous
                                </Link>
                        }
                    
                        {
                            ...
                            (nextPage === true) ? "" : 
                                <Link  className = "paginationLink" to={`/comics/page/${currentPage + 1}`} onClick={onClickNext}>
                                Next
                                </Link>
                        }
                    <br />
